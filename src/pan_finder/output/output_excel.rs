...
fn output_found_pan(
    ...
    if result.nb_found_pan != 0 {
        ...
        worksheet_pan.autofilter(0, 0, u32::try_from(result.nb_found_pan).unwrap(), 2)?;
        ...
        worksheet_pan.write_with_format(0, 1, "Brand", header_format)?;
        worksheet_pan.write_with_format(0, 2, "PAN", header_format)?;
        ...
        for item in &result.results_list {
            if !item.pan_found.is_empty() {
                for pan in &item.pan_found {
                    ...
                    worksheet_pan.write_with_format(row, 1, &pan.brand, cell_format)?;
                    worksheet_pan.write_with_format(row, 2, &pan.pan, cell_format)?;
                    row += 1;
                }
            }
        }
